...
Breadcrumbs::for(
    'editorArticles',
    function ($trail, $user_role, $user_id, $article_status) {
        $breadcrumbs_title = "";
        if (!empty($article_status)) {
            $breadcrumbs_title = $article_status;
        }
        $trail->parent('home');
        $trail->push($breadcrumbs_title, route('editorArticles', ['role' => $user_role, 'id' => $user_id, 'status' => $article_status]));
    }
);

Breadcrumbs::for(
    'editorArticleDetail',
    function ($trail, $article, $user_role, $user_id, $status, $slug) {
        $breadcrumbs_title = "";
        if (!empty($article->title)) {
            $breadcrumbs_title = $article->title;
        }
        $trail->parent('editorArticles', $user_role, $user_id, $status);
        $trail->push($breadcrumbs_title, route('editorArticles', ['role' => $user_role, 'id' => $user_id, 'status' => $status, 'slug' => $slug]));
    }
);
...
Breadcrumbs::for(
    'authorArticles',
    function ($trail, $author_id, $status) {
        $breadcrumbs_title = "";
        if (!empty($status)) {
...
Breadcrumbs::for(
    'reviewerArticles',
    function ($trail, $reviewer_id, $status) {
        $breadcrumbs_title = "";
        if (!empty($status)) {
            $breadcrumbs_title = $status;
        }
...
Breadcrumbs::for(
    'managePages',
    function ($trail, $user_role) {
        $trail->parent('home');
        $trail->push(trans('prs.manage_pages'), route('managePages', ['role' => $user_role]));
    }
);

Breadcrumbs::for(
    'createPage',
    function ($trail, $user_role, $user_id) {
        $trail->parent('managePages', $user_role);
        $trail->push(trans('prs.create_page'), route('createPage', ['role' => $user_role, 'userId' => $user_id]));
    }
);

Breadcrumbs::for(
    'editPage',
    function ($trail, $user_role, $id) {
        $trail->parent('managePages', $user_role);
        $trail->push(trans('prs.edit_page'), route('editPage', ['role' => $user_role, 'id' => $id]));
    }
);

Breadcrumbs::for(
    'showPage',
    function ($trail, $page, $slug) {
        $page_title = "";
        if (!empty($page->title)) {
