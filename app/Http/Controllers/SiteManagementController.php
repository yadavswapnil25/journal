...
class SiteManagementController extends Controller
{
    ...
    public function index($userRole = "")
    {
        ...
        $languages = Arr::pluck(Helper::getTranslatedLang(), 'title', 'code');
    ...
    public function storeResourceMenuPages(Request $request)
    {
        ...
        if (!empty($r_pages)) {
            foreach ($r_pages as $page) {
                $pages[] = DB::table('pages')->select('title', 'slug')->where('slug', $page)->first();
            }
            DB::table('sitemanagements')->insert(
                [
    ...
    {
        $response = array();
        $requested_image = DB::table('sitemanagements')->where('meta_key', 'logo')->select('meta_value')->first();
    ...
    {
        $response = array();
        $requested_image = DB::table('sitemanagements')->where('meta_key', 'advertisement')->select('meta_value')->first();
        if (!empty($requested_image)) {
            $image = UploadMedia::getImageName($requested_image->meta_value);
            $response['image'] = $image;
            return $response;
    ...
    public function createEmailSetting()
    {
        ...
        return view('admin.cms.email_settings', compact('existing_email_settings'));
    }
    ...
    public function storeLanguageSetting(Request $request)
    {
        ...
        if (!empty($language)) {
            if (!empty($language) && File::exists(lang_path() . '/' . $language)) {
                if (File::exists(lang_path($language . '/prs.php'))
                    && File::exists(lang_path($language . '/auth.php'))
                    && File::exists(lang_path($language . '/pagination.php'))
                    && File::exists(lang_path($language . '/passwords.php'))
                    && File::exists(lang_path($language . '/validation.php'))
                ) {
                    Helper::changeEnv(
                        [
                            'APP_LANG' => $language,
    ...
    {
        $json = array();
        $settings = DB::table('sitemanagements')->select('meta_value')->where('meta_key', 'show-page-' . $request['page_id'])->first();
