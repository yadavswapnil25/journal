...
class SettingController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.settings.account-settings', compact('user_id'));
    }
        ...
        if ($request['deleted_image'] == 'deleted') {
            ...
            if ($extension === "jpg" || $extension === "png") {
                ...
                if (!file_exists($path . $user_id)) {
                    $directory = File::makeDirectory($path . $user_id, 0755, true);
                }
                // generate mini image
                ...
                $user = User::find($user_id);
                $user->user_image = htmlspecialchars($filename, ENT_QUOTES, 'UTF-8');
                $user->save();
                $response = array();
