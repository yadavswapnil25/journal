...
class SiteManagement extends Model
{
    ...
    public static function getMetaValue($meta_key)
    {
        ...
        if (!empty($meta_key)) {
            if ($meta_key === 'logo') {
                $logo = '';
                $uploaded_logo = DB::table('sitemanagements')->select('meta_value')->where('meta_key', 'logo')->first();
            ...
            if ($meta_key === 'advertisement') {
                $img = '';
                $uploaded_image = DB::table('sitemanagements')->select('meta_value')->where('meta_key', 'advertisement')->first();
                ...
                $serialize_array = DB::table('sitemanagements')->select('meta_value')->where('meta_key', $meta_key)->first();
